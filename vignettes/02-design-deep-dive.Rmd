...
df = open_dataset('s3://lynker-spatial/v20.1/conus_net.parquet') |> 
  select('hl_uri') |>
  distinct() |> 
  collect() |>
  na.omit() |>
  tidyr::separate(col = "hl_uri", sep = "-", into = c("Type", "id")) |>
  group_by(Type) |>
  tally(name = "Count")
...
reference_gpkg = get_fabric(VPU = gage$VPUID, 
...
nex = read_sf(reference_gpkg, "hydrolocations")
head(nex)
```
...
dbListTables(db)
mpoi = collect(tbl(db, "hydrolocations"))
