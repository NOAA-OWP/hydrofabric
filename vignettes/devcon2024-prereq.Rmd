...
1- Cloning the Rocker Versioned2 Repository
...
```
We have provided a preconfigured docker image that can be obtained via ...
...
```bash
docker build -f .\hydrofabric.Dockerfile -t hydrofabric .\rocker-versioned2
```

To launch the Docker image and start R and RStudio, run the following command

```bash
docker run -it --rm -p 8686:8686 -p 2200:2200 -e DISABLE_AUTH=true -e ROOT=TRUE hydrofabric
```

Once the Docker container is running, access RStudio by navigating to `localhost:8686` in your web browser.
