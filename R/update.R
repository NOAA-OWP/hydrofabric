...
#' }
#' 
hydrofabric_update <- function(recursive = FALSE, repos = getOption("repos")) {
  
  deps <- hydrofabric_deps(recursive, repos)
  behind <- dplyr::filter(deps, behind)
  
  if (nrow(behind) == 0) {
    cli::cat_line("All hydrofabric packages up-to-date")
    return(invisible())
  }
...
hydrofabric_deps <- function(recursive = FALSE, repos = getOption("repos")) {
  pkgs <- utils::available.packages(repos = repos)
  deps <- tools::package_dependencies("hydrofabric", pkgs, recursive = recursive)
