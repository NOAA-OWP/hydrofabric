source('workflow/nhd_workflows/01_read.R')
source('workflow/nhd_workflows/02_frag_fix.R')
source('workflow/nhd_workflows/03_levelpath.R')
source('workflow/nhd_workflows/04_island_removal.R')
source('workflow/nhd_workflows/05_dissolve_headwaters.R')
...
path  <- "workflow/nhd_workflows/cache/ngen_01a-4.gpkg"
...
fline = read_sf(path, "flowpaths") %>%
  select(ID = comid, toID = tocomid, lengthkm, levelpathi) %>%
  mutate(from_nID = ID)
...
catchment_data = read_sf(path, "catchments") %>%
  select(ID = comid, areasqkm) %>%
  mutate(ID = paste0(catchment_prefix, ID)) %>%
  left_join(catchment_edge_list, by = "ID")

flowpath_data = select(fline, ID, lengthkm, main_id = .data$levelpathi) %>%
  mutate(ID = paste0(catchment_prefix, ID)) %>%
  left_join(catchment_edge_list, by = "ID")
