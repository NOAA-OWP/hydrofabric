...
bb = st_bbox(filter(fl_inputs$og_fl, comid == 22741689) %>% 
               st_buffer(2000))
...
g1 = ggplot() +
  ...
  geom_sf(data = fl_inputs$og_fl, aes(color = "Flowlines"), lwd = .5) + 
  geom_sf(data = fl_inputs$flowpaths, aes(color = "Flowpaths"), lwd = .8) +
  scale_color_manual(labels = c("Flowlines", "Flowpaths"),
                     values = c("red", "blue")) +
  labs(color = "Network Type") +
  geom_sf(data = st_as_sf(st_as_sfc(bb)), fill  = NA, lwd = 1) + 
  # geom_sf(data = fl_inputs$pois, color = "black", size = 3) + 
  # geom_sf(data = fl_inputs$pois, color = "darkgreen", size = 2) +
  theme_void() + 
  theme(legend.position="bottom") 
...
g2 = ggplot() +
  ...
  scale_color_manual(labels = c("Flowlines", "Flowpaths"),
                     values = c("red", "blue")) +
  labs(color = "Network Type") +
  coord_sf(xlim = c(bb$xmin, bb$xmax), # min & max of x values
...
fish_sample = st_filter(fl_inputs$og_fl, st_as_sfc(bb)) %>% 
  filter(comid != div_id) %>% 
  st_union() %>% 
  st_line_merge() %>% 
  st_cast("LINESTRING") %>% 
...
div = filter(fl_inputs$og_fl, comid == div_id) %>% 
  st_line_sample(n = 2) %>% 
  st_cast("POINT") %>% 
  st_as_sf() %>% 
  mutate(Type = c("Fish Sample", "Velocity Measurement")) %>% 
  rename_geometry("geometry")

pts = bind_rows(hucs, fish_sample, div) %>% 
  st_cast("POINT")
