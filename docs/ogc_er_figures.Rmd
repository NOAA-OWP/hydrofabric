...
Similar figure 1 to above but with a fish sample, a water quality sample, and a velocity measurement on a diverted channel.

# Flowlines

Show the distributary system of flowlines with one primary flowpath flowing from catchment inlet to outlet.

```{r, echo = FALSE}
flowlines = st_intersection(get_nhdplus(AOI = tmp), tmp)

pts = bind_rows(get_node(tmp_fl, "start"), get_node(tmp_fl, "end")) 
pts$Type = c("Inlet", "Outlet")
...
ggplot() +
  ...
  scale_fill_distiller(type = "seq",
                       ...
                       palette = "Greys", guide = "none") +
  geom_sf(data = tmp, fill = NA, lwd = 1) + 
  geom_sf(data = flowlines, color = "blue", lwd = 1) + 
  geom_sf(data = tmp_fl, color = "navy", lwd = 2) +
  geom_sf(data = pts, aes(color = Type), size = 5) + 
  labs(color = "Flowpath Position") +
  theme_void() + 
  theme(legend.position="bottom")
```
